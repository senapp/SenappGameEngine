...
namespace Senapp.Engine.Raycasts
{
    ...
    {
        public static float DistanceFromPoint(Vector2 mouseLocation, Vector3 target)
        {
            ...
            Vector3 pt = ClosestPoint(near, far, target); // find point on ray which is closest to test point

            return Vector3.Distance(pt, target); // return the distance
        }
